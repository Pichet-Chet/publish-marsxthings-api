...
{
	public class SysCustomerMachineFilter : Pagination
	{
		public SysCustomerMachineFilter()
		{
        ...
        public static List<SysCustomerMachineModel> ApplySorting(List<SysCustomerMachineModel> queryable, string sortName, string sortType)
        {
            if (!string.IsNullOrEmpty(sortName))
            {
                PropertyInfo propertyInfo = typeof(SysCustomerMachineModel).GetProperty(sortName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);

                if (propertyInfo != null)
                {
                    var parameter = Expression.Parameter(typeof(SysCustomerMachineModel), "x");
                    var property = Expression.Property(parameter, propertyInfo);
                    var lambda = Expression.Lambda<Func<SysCustomerMachineModel, object>>(
